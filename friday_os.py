...
def boot_react_hud():
    ...
    Token is NO LONGER injected into .env — the neural_router /livekit-token
    endpoint mints a fresh JWT at runtime so the frontend never holds a stale token.
    """
...
class FridayHUD(ctk.CTk):
    def __init__(self):
        super().__init__()
        self.title("F.R.I.D.A.Y. Mark VI - Master Control")
    ...
    def build_ui(self):
        self.header = ctk.CTkLabel(self, text="F.R.I.D.A.Y. NUCLEUS",
                                   font=("Courier New", 28, "bold"), text_color="#00ffff")
        self.header.pack(pady=(30, 10))

        self.status_frame = ctk.CTkFrame(self, fg_color="#111111",
                                         border_color="#333333", border_width=1)
        self.status_frame.pack(pady=20, padx=40, fill="x")

        self.brain_status = ctk.CTkLabel(self.status_frame,
                                         text="Cognitive Core: OFFLINE",
                                         font=("Courier New", 14), text_color="#ff4444")
        self.brain_status.pack(pady=5, anchor="w", padx=20)

        self.comms_status = ctk.CTkLabel(self.status_frame,
                                         text="Neural Bridge: OFFLINE",
                                         font=("Courier New", 14), text_color="#ff4444")
        self.comms_status.pack(pady=5, anchor="w", padx=20)

        self.context_status = ctk.CTkLabel(self.status_frame,
                                           text="Active Context: NONE",
                                           font=("Courier New", 12), text_color="#aaaaaa")
        self.context_status.pack(pady=5, anchor="w", padx=20)

        self.swarm_frame = ctk.CTkFrame(self, fg_color="#0a0a0a",
                                        border_color="#005555", border_width=1)
        self.swarm_frame.pack(pady=10, padx=40, fill="x")

        self.swarm_title = ctk.CTkLabel(self.swarm_frame, text="SWARM NODE TELEMETRY",
                                        font=("Courier New", 14, "bold"), text_color="#55aaaa")
        self.swarm_title.pack(pady=(10, 5))

        self.node_architect = ctk.CTkLabel(self.swarm_frame, text="[ARCHITECT]: OFFLINE",
                                           font=("Courier New", 12), text_color="#555555")
        self.node_architect.pack(anchor="w", padx=20)

        self.node_intel = ctk.CTkLabel(self.swarm_frame, text="[INTEL]: OFFLINE",
                                       font=("Courier New", 12), text_color="#555555")
        self.node_intel.pack(anchor="w", padx=20)

        self.node_sentinel = ctk.CTkLabel(self.swarm_frame, text="[SENTINEL]: OFFLINE",
                                          font=("Courier New", 12), text_color="#555555")
        self.node_sentinel.pack(anchor="w", padx=20, pady=(0, 10))

        self.terminal_label = ctk.CTkLabel(self, text="SYSTEM LOGS",
                                           font=("Courier New", 12, "bold"), text_color="#555555")
        self.terminal_label.pack(anchor="w", padx=40)

        self.terminal = ctk.CTkTextbox(self, height=200, fg_color="#050505",
                                       text_color="#00dd00", font=("Consolas", 12))
        ...
        self.boot_btn = ctk.CTkButton(
            self, text="IGNITE SYSTEMS",
            font=("Courier New", 16, "bold"),
            fg_color="#222222", hover_color="#005555",
    ...
    def track_active_context(self):
        ...
        try:
            ...
            self.context_status.configure(
                text=f"Context: {title} | CPU: {cpu_usage}% | RAM: {ram_usage}%")
    ...
    def boot_sequence(self):
        self.is_online = True
        self.boot_btn.configure(text="SHUTDOWN SYSTEMS",
                                fg_color="#550000", hover_color="#ff0000",
                                border_color="#ff4444")
        self.header.configure(text_color="#00ffff")
        self.log("Initiating Mark VI Boot Sequence...")
    ...
    def shutdown_sequence(self):
        self.is_online = False
        self.boot_btn.configure(text="IGNITE SYSTEMS",
                                fg_color="#222222", hover_color="#005555",
                                border_color="#00ffff")
